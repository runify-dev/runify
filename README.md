...
传统 AI Agent 是黑盒 — 你给它一个目标，它自己想办法，你只能看最终结果。出了问题，你不知道哪一步出了错。

Runify 不同。每个 Agent 都是一个**可视化工作流**，由你亲手编排：

- **看到每一步** — 节点级别的执行过程，输入输出一目了然
- **改任何一步** — 提示词、工具调用、判断逻辑，随时调整
- **调试任何一步** — 单步执行、断点、变量检查
- **复用任何一步** — 子流程封装，像搭积木一样组合

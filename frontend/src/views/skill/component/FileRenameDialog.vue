<template>
  <Dialog v-model:visible="visible" modal :header="t('skill.file.renameHeader')" :style="{ width: '25rem' }">
    <div class="flex flex-col gap-4">
      <div class="flex flex-col gap-1.5">
        <label class="text-sm font-medium text-surface-700">{{ t('skill.file.nameLabel') }}</label>
    ...
    <template #footer>
      ...
      <Button :label="t('common.confirm')" :disabled="!name.trim()" @click="submit"/>
    </template>
  </Dialog>
</template>

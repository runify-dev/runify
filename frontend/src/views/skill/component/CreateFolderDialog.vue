<template>
  <Dialog v-model:visible="visible" modal :header="t('skill.file.createFolderHeader')" :style="{ width: '25rem' }">
    <div class="flex flex-col gap-4">
      <div class="flex flex-col gap-1.5">
        <label class="text-sm font-medium text-surface-700">{{ t('skill.file.folderNameLabel') }}</label>
        <InputText v-model="name" type="text" :placeholder="t('skill.file.folderNamePlaceholder')" fluid class="!text-sm" autofocus @keyup.enter="submit"/>
      </div>
    </div>
    <template #footer>
...
const open = (options: { skillId: string; parentId?: string }) => {
  ...
  name.value = t('skill.file.newFolderName')
  visible.value = true
}

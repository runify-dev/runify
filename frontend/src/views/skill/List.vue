  ...
  <div class="p-4 ">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      <template v-for="item in filteredList" :key="item.id">
        ...
        >
          ...
          <template #content>
            ...
            <p class="text-xs text-surface-500 leading-relaxed line-clamp-2 min-h-[2.5rem]">{{ item.desc || t('skill.noDesc') }}</p>
          </template>
          <template #footer>
      ...
      >
        ...
        <p class="text-sm">{{ t('skill.empty') }}</p>
      </div>
    </div>
...
const permissionCreate = computed(() => {
  return hasPermission([PermissionConstants.SKILL_CREATE.newResourcePermission(folderId.value), Role.ADMIN], 'OR')
})
...
const createMenuItems = computed(() => [
  {
    label: t('skill.createSkill'),
    description: t('skill.createSkillDesc'),
    icon: 'pi pi-bolt',
    command: () => bus.emit('open:create:skill:dialog', folderId.value)
  },
  {
    label: t('skill.importSkill'),
    description: t('skill.importSkillDesc'),
    icon: 'pi pi-upload',
...
const menuItems = computed(() => [
  {
    label: t('skill.export'),
    visible: hasPermission([
      PermissionConstants.SKILL_READ.newResourcePermission(activeItem.value?.id || ''),
      Role.ADMIN], "OR"),
    icon: 'pi pi-download',
  ...
  {separator: true},
  {
    ...
    visible: hasPermission([
      PermissionConstants.SKILL_DELETE.newResourcePermission(activeItem.value?.id || ''),
      Role.ADMIN], "OR"),
    icon: 'pi pi-trash',
...
const handleImport = (event: Event) => {
  ...
  treeCommonAPI.importResource(folderId.value, file).then(() => {
    lisResource()
    toast.add({severity: 'success', summary: t('skill.importSuccess'), life: 2000})
    input.value = ''
  })
}
...
const handleDelete = (item: Node) => {
  confirm.require({
    message: t('skill.deleteMessage', {name: item.name}),

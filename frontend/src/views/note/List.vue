<template>
  <div class="p-4 ">
    ...
    <div class="flex items-center justify-between gap-4 mb-6">
      ...
      <!-- 新建按钮 -->
      <Button icon="pi pi-plus" v-if="permissionCreate" label="新建笔记" @click="openCreate"
              class="shrink-0"/>
    </div>
        ...
        >
          ...
          <template #footer>
            <div class="flex items-center justify-between pt-2.5 border-t" style="border-color: var(--p-content-border-color);">
              <span class="text-[11px] font-medium px-2 py-0.5 rounded-full bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-300">笔记</span>
...
const permissionCreate = computed(() => {
  return hasPermission([PermissionConstants.NOTE_CREATE.newResourcePermission(folderId.value), Role.ADMIN], 'OR')
})
...
const menuItems = computed(() => [
  {
    ...
    visible: hasPermission([
      PermissionConstants.NOTE_READ.newResourcePermission(activeItem.value?.id || ''),
      Role.ADMIN], "OR"),
  ...
  {separator: true},
  {
    label: '删除',
    visible: hasPermission([
      PermissionConstants.NOTE_DELETE.newResourcePermission(activeItem.value?.id || ''),
      Role.ADMIN], "OR"),
    icon: 'pi pi-trash',

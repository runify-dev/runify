<template>
    <div class="p-10 pt-5 w-full h-full">
        <div
        ...
        <MdEditor class="rounded-xl" :editorId="state.id" @onUploadImg="onUploadImg" :onGetCatalog="onGetCatalog"
            ref="mdEditorRef" v-model="knowledge.content" :toolbars="toolbars" previewTheme="default">
        </MdEditor>
    </div>
</template>
...
const goDetails = () => {
    router.push({ name: "knowledgeDetails", params: { folderId: folderId.value, id: resourceId.value } })
}
...
const edit = () => {
    KnowledgeApi.edit(folderId.value ? folderId.value : 'root', resourceId.value, { content: knowledge.value.content }).then(ok => {
        goDetails()
    })

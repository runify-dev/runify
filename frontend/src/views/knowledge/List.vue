...
const permissionCreate = computed(() => {
  return hasPermission([PermissionConstants.KNOWLEDGE_CREATE.newResourcePermission(folderId.value), Role.ADMIN], 'OR')
})
...
const handleOpen = (item: Node) => {
  router.push({name: 'knowledgeDocFolder', params: {id: item.id, folderId: ROOT_FOLDER_ID}})

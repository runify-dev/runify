<template>
  <div class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white">
      ...
      <table class="min-w-full border-collapse text-sm">
        <thead class="sticky top-0 z-10 bg-slate-50">
          <tr class="text-slate-500">
            <th class="border-b border-slate-200 px-4 py-3 text-left text-xs font-medium">姓名</th>
            <th class="border-b border-slate-200 px-4 py-3 text-left text-xs font-medium">
              用户名
            </th>
            <th
              class="w-[96px] border-b border-slate-200 px-4 py-3 text-center text-xs font-medium"
            >
              操作
            </th>
          </tr>
        ...
        <tbody v-else>
          <tr>
            <td colspan="3" class="px-6 py-14 text-center text-sm text-slate-400">
    ...
    <div class="flex items-center justify-end gap-3 border-t border-slate-200 px-4 py-3 text-sm">
      <span class="text-slate-500">共 {{ memberPage.total }} 条</span>

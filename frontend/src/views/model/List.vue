<template>
    <el-row :gutter="10" class="p-8">
        <el-col :span="8"> <el-input v-model="searchText" style="max-width: 600px" placeholder="搜索应用"
        ...
        <el-col :span="8"><el-button class="float-right">新建模型</el-button></el-col>
    </el-row>
    <el-row :gutter="10" class="pr-8 pl-8">
        <el-col v-for="node in nodeList" :key="node.id" :xs="24" :sm="12" :md="8" :lg="6" :xl="6" class="pt-4">
            <CardBox :title="node.name"
                @click="router.push({ name: 'modelDetails', params: { folderId: node.parentId ? node.parentId : 'root', id: node.id } })">
                <template #description>
                    <div v-if="node.desc">
                        <MdPreview :modelValue="node.desc">
                        </MdPreview>
                    </div>
...
const listApplication = () => {
    NodeApi.listResource('model', resourceId.value).then(ok => {

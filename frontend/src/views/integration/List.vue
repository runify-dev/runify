<template>
  <div class="p-4 ">
    ...
    <div class="flex items-center justify-between gap-4 mb-6">
      ...
      <!-- 新建按钮 -->
      <Button icon="pi pi-plus" v-if="permissionCreate" :label="t('integration.create')" @click="openCreate"
              class="shrink-0"/>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        >
          <template #header>
            <div class="flex items-start justify-between p-4 pb-0">
              <div class="w-10 h-10 rounded-xl bg-primary-50 flex items-center justify-center text-primary-500 text-lg shrink-0"><i class="pi pi-share-alt"/></div>
          ...
          <template #content>
            ...
            <p class="text-xs text-surface-500 leading-relaxed line-clamp-2 min-h-[2.5rem]">{{ item.desc || t('integration.noDesc') }}</p>
          </template>
          <template #footer>
            <div class="flex items-center justify-between pt-2.5 border-t" style="border-color: var(--p-content-border-color);">
              <span class="text-[11px] font-medium px-2 py-0.5 rounded-full bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-300">{{ typeLabel(item.type) }}</span>
      ...
      >
        ...
        <p class="text-sm">{{ t('integration.empty') }}</p>
      </div>
    </div>
...
import {getTypeMeta} from '@/api/integration'
...
const typeLabel = (type?: string) => (type ? getTypeMeta(type)?.label || type : t('integration.integrationLabel'))
...
const permissionCreate = computed(() => {
  return hasPermission([PermissionConstants.INTEGRATION_CREATE.newResourcePermission(folderId.value), Role.ADMIN], 'OR')
})
...
const menuItems = computed(() => [
  {
    label: t('integration.open'),
    icon: 'pi pi-arrow-up-right',
    visible: hasPermission([
      PermissionConstants.INTEGRATION_READ.newResourcePermission(activeItem.value?.id || ''),
      Role.ADMIN], "OR"),
  ...
  {separator: true},
  {
    ...
    visible: hasPermission([
      PermissionConstants.INTEGRATION_DELETE.newResourcePermission(activeItem.value?.id || ''),
      Role.ADMIN], "OR"),
    icon: 'pi pi-trash',
...
const openCreate = () => {
  bus.emit('open:create:integration:dialog', folderId.value)
}

const handleOpen = (item: Node) => {
  router.push({name: 'integrationDetails', params: {id: item.id}})
  bus.emit('sidebar:flip')
}

const handleDelete = (item: Node) => {
  confirm.require({
    message: t('integration.deleteMessage', { name: item.name }),
    header: t('integration.deleteConfirm'),
    ...
    accept: () => {
      treeCommonAPI.removeResource(item.id).then(() => {
        ...
        toast.add({severity: 'success', summary: t('integration.deleteSuccess'), life: 2000})
      })
    }
  })
}

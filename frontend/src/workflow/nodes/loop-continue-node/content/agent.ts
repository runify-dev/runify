...
/** loop-continue-node（循环跳过）的 AI 生成配置（命令式，手写）。仅循环子画布内可用。 */
...
function useAgent(bindings: AgentBindings) {
  ...
  return useAgentConfig(
    {
      description: '（仅循环子画布内）满足条件时跳过本轮，进入下一轮',
      prompt:
        'loop-continue-node（循环跳过）：用 update 配置。\n' +
      ...
      tools: [
        {
          name: 'update',
          description: '配置当前循环跳过节点：触发条件与逻辑。',
          parameters: input,
          apply: (args) => {

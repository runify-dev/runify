...
const page: (
  ...
  pageSize: number,
  loading?: Ref<boolean>
) => Promise<Result<Page<User>>> = (query, currentPage, pageSize, loading) => {
  return get(`/user`, { ...query, currentPage, pageSize }, loading)
}

const listUser: (
  query: { global?: string; username?: string; nikename?: string },
  loading?: Ref<boolean>
) => Promise<Result<Page<User>>> = (query, loading) => {
  return get(`/user`, query, loading)
}
...
const logout: (loading?: Ref<boolean>) => Promise<Result<User>> = (loading) => {
  return get(`/logout`, undefined, loading)
}

const createUser: (body: any, loading?: Ref<boolean>) => Promise<Result<User>> = (
  body,
  loading
) => {
  return post(`/user`, body, {}, loading)
}
const deleteUser: (userId: string, loading?: Ref<boolean>) => Promise<Result<boolean>> = (
  userId,
  loading
) => {

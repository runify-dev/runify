...
const hasPermissionChild = (
  ...
  if (permission instanceof Permission) {
    return (
      permissions.has(permission.toPermissionKey()) &&
      ((permissions.get(permission.toPermissionKey()) as bigint) & permission.bit()) > 0
    )
  }

  if (permission instanceof AggregatePermission) {
    return permission.hasPermission(roles, permissions)
  }

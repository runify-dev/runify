<template>
  <el-aside class="app-aside">
    ...
    <div class="menu-contaner">
      <Menu :menu="menu" v-for="(menu, index) in menuList" :key="index"> </Menu>
    </div>
      ...
      <el-popover trigger="click" popper-class="system-person" placement="right">
        ...
        <div class="w-[200px] shadow-md border border-[#dee0e3] bg-white rounded-md">
          ...
          <div class="flex items-center h-[62px] px-2 mb-1 border-b border-[#dee0e3]">
            <el-avatar :src="`${baseURL}/user.jpeg`" class="mr-4" />
          ...
          <div class="flex flex-col">
            <div
              class="flex items-center h-8 px-2 cursor-pointer rounded hover:bg-gray-100 active:bg-gray-200"
              @click="toSystem"
            >
            ...
            <div
              class="flex items-center h-8 px-2 my-1 cursor-pointer rounded hover:bg-gray-100 active:bg-gray-200 border-t border-[#dee0e3]"
              @click="logout"
            >
...
const baseURL = computed(() => {
  return window.RUNIFY_APP.admin.baseURL
})
const { user } = useStore()

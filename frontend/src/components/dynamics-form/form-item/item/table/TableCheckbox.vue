...
import TableColumn from '@/components/dynamics-form/form-item/item/table/TableColumn.vue'
...
const labelField = computed(() => {
  return props.formField.label_field ? props.formField.label_field : 'key'
})
  ...
  if (props.modelValue) {
    ...
    if (rows) {
      if (rows.length > 3) {
        return (
          rows
            .map((row) => row[labelField.value])
            .splice(0, 3)
            .join(',') + '...'
        )
      } else {
        return rows.map((row) => row[labelField.value]).join(',')
      }
    }
  }
  return props.modelValue

    ...
    <button class="tc-head" @click="$emit('toggle')">
      ...
      <span class="tc-icon" :class="{ spin: loading }">
        ...
        <svg v-else viewBox="0 0 16 16" width="16" height="16" fill="none">
          <path d="M8 2v8M4 7l4 4 4-4" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>
          <path d="M2 13h12" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>
        </svg>
      </span>
      <span class="tc-name">{{ skillName || 'download_skills' }}</span>
...
const skillName = computed(() => {
  ...
  if (!raw) return ''
  return extractPartialJsonField(raw, 'skill_name') || extractPartialJsonField(raw, 'skill_id') || ''
})

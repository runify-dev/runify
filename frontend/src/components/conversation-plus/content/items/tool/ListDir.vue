    ...
    <button class="tc-head" @click="$emit('toggle')">
      ...
      <span class="tc-icon" :class="{ spin: loading }">
        ...
        <svg v-else viewBox="0 0 16 16" width="16" height="16" fill="none">
          <path d="M2 4.5A1.5 1.5 0 0 1 3.5 3h2.086a1.5 1.5 0 0 1 1.06.44l.354.354a.5.5 0 0 0 .354.146H12.5A1.5 1.5 0 0 1 14 5.44V11.5a1.5 1.5 0 0 1-1.5 1.5h-9A1.5 1.5 0 0 1 2 11.5v-7Z" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/>
        </svg>
      </span>
      <span class="tc-name">{{ dirPath || 'list_dir' }}</span>
...
const dirPath = computed(() => {
  ...
  if (!raw) return ''
  return extractPartialJsonField(raw, 'path')
})

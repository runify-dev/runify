 ...
 */
export function enhanceTreeWithPaths(treeData, options: Options = {}) {
    ...
    // 内部处理函数
    const processNode = (node, parentPath = [], parentValuePath = []) => {
        // 获取节点的标签和值
 ...
 */
export function findNodeByValuePath(treeData, valuePath, options: Options = {}) {
    const { valueKey = 'value', childrenKey = 'children' } = options;

    if (!valuePath || valuePath.length === 0) return null;

    const findInNodes = (nodes, depth = 0) => {
 ...
 */
export function findNodeAncestors(treeData, valuePath, options: Options = {}) {
    const { valueKey = 'value', childrenKey = 'children' } = options;

    const ancestors = [];

    const findRecursive = (nodes, depth = 0) => {
        for (const node of nodes) {
            const nodeValue = node[valueKey] || node.value;
 ...
 */
export function findAllLeafNodes(treeData, options: Options = {}) {
 ...
 */
export function filterTreeNodes(treeData, predicate, options: Options = {}) {
    const { childrenKey = 'children' } = options;
 ...
 * @returns {string} 显示文本
 */
export function getDisplayTextByValuePath(treeData, valuePath, options: Options = {}) {

<template>
  ...
  >
    ...
    <input style="display: none" ref="fileUploadRef" type="file" accept="image/*" multiple />
  </button>
</template>
...
const fileUploadRef = ref<InstanceType<typeof HTMLInputElement>>()

const focus = () => {
  fileUploadRef.value?.click()
}
onMounted(() => {
  fileUploadRef.value?.addEventListener('change', function (e: any) {
    if (!e.target?.files) {
      return
    }

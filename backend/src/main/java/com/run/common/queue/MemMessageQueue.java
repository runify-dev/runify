 ...
 * {@code @注释: }
 */
public class MemMessageQueue implements MessageQueue<MessageChunk> {
    private final static ConcurrentMap<String, Instance> eventMap = new ConcurrentHashMap<>();

    @Override
    public void consume(String eventId, String consumerId, Consumer<MessageChunk> call) {
        eventMap.get(eventId).consume(consumerId, call);
    }

    @Override
    public void publish(String eventId, MessageChunk message) {
    ...
    class Instance {
        private final List<MessageChunk> messageLog = new CopyOnWriteArrayList<>();
        ...
        public void publish(MessageChunk message) {
            // 1. 将消息追加到日志末尾
        ...
        public void consume(String consumerId, Consumer<MessageChunk> call) {
            ...
            if (currentOffset < messageLog.size()) {
                // 从日志中获取该偏移量对应的消息
                MessageChunk messageChunk = messageLog.get((int) currentOffset);
                // 成功消费后，将该消费者的偏移量加1
